...
const successiveDivide = (divisor: number, xs: number[]) => foldr(succesiveDivideOp, divisor, xs);
assert.equal(successiveDivide(2, [16, 8, 4]), 4);

const successiveDivideFoldrL = (divisor: number, xs: number[]) => foldrL(succesiveDivideOp, divisor, xs);
